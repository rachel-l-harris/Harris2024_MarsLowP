...
#SBATCH -J array_H2CO2_30C_1.5atm_blastn_ncbi-all-CDS
#SBATCH -n 1 #--ntasks
#SBATCH -c 4 #--cpus_per-task
#SBATCH -N 1 #node count
#SBATCH -t 0-168:00:00 #--run time in hours
#SBATCH -p bigmem_intermediate #--partition
#SBATCH --mem=300G #--memory request in GB
#SBATCH -o /n/holyscratch01/girguis_lab/rachel_harris/Mars_low-pressure/H2CO2/30C_1.5atm/cds_KEGG-mby-all-CDS/mappedReads/blastn/array_H2CO2_30C_1.5atm_blastn_ncbi-all-CDS_mapping_%A_%a.out
#SBATCH -e /n/holyscratch01/girguis_lab/rachel_harris/Mars_low-pressure/H2CO2/30C_1.5atm/cds_KEGG-mby-all-CDS/mappedReads/blastn/array_H2CO2_30C_1.5atm_blastn_ncbi-all-CDS_mapping_%A_%a.err
#SBATCH --array=0-5
...
echo ""
echo "####################################"
echo "BLASTn H2CO2/30C_1.5atm MAPPED READS"
echo "#####################################"
echo ""
echo "cd H2CO2/30C_1.5atm/cds_KEGG-mby-all-CDS/mappedReads/mappers"
cd H2CO2/30C_1.5atm/cds_KEGG-mby-all-CDS/mappedReads/mappers
echo ""

echo "Load the sample names into a bash array"
echo "samples=(A1-30C2 A2-30C2 A3-30C1 A3-30C2 A4-30C1 A4-30C2)"
samples=(A1-30C2 A2-30C2 A3-30C1 A3-30C2 A4-30C1 A4-30C2)
echo ""
...
echo "blast_formatter -archive ../blastn/mappers_${samples[$SLURM_ARRAY_TASK_ID]}.BLASTn.asn -outfmt "5 qseqid qlen sseqid slen qstart qend sstart send length pident nident mismatch evalue bitscore staxids saccver stitle" -out ../blastn/mappers_${samples[$SLURM_ARRAY_TASK_ID]}_R1_KEGG-mby-all-CDS.BLASTn.out"
...
echo "blast_formatter -archive ../blastn/mappers_${samples[$SLURM_ARRAY_TASK_ID]}.BLASTn.asn -outfmt "5 qseqid qlen sseqid slen qstart qend sstart send length pident nident mismatch evalue bitscore staxids saccver stitle" -out ../blastn/mappers_${samples[$SLURM_ARRAY_TASK_ID]}_R2_KEGG-mby-all-CDS.BLASTn.out"
